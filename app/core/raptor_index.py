...
TREE_DIR = os.path.join(
    os.path.dirname(__file__), "..", "..", "data", "raw", "paper_trees"
)
...
def load_tree(arxiv_id: str) -> Optional[nx.DiGraph]:
    ...
    with open(path, "rb") as f:
        return pickle.load(f)  # nosec B301 - trusted local RAPTOR artifacts only
...
def get_topics(G: nx.DiGraph) -> List[Dict[str, Any]]:
    ...
    for node in G.successors("root"):
        ...
        if n.get("type") == "topic":
            sections = [
                s for s in G.successors(node) if G.nodes[s].get("type") == "section"
            ]
            topics.append(
                {
                    "node_id": node,
...
def get_sections(G: nx.DiGraph, parent_node: str = None) -> List[Dict[str, Any]]:
    ...
    for node in candidates:
        ...
        if n.get("type") == "section":
            chunk_count = sum(
                1 for c in G.successors(node) if G.nodes[c].get("type") == "chunk"
            )
            sections.append(
                {
                    "node_id": node,
...
def get_tree_structure(arxiv_id: str) -> Optional[Dict[str, Any]]:
    ...
    if not topics:
        ...
        structure = {
            **info,
            ...
            "sections": sections,
            "total_chunks": sum(
                1 for n in G.nodes if G.nodes[n].get("type") == "chunk"
            ),
    ...
    structure = {
        **info,
        ...
        "total_sections": sum(
            1 for n in G.nodes if G.nodes[n].get("type") == "section"
        ),
